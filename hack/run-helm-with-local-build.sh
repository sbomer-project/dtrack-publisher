...
LOCAL_CHART_PATH="helm/dtrack-publisher-chart"
...
echo "--- Building local dtrack-publisher image inside of Minikube ---"
# Will have local image dtrack-publisher:latest in minikube
bash ./hack/build-local-dtrack-publisher-into-minikube.sh
...
target_name = 'dtrack-publisher-chart'
...
for line in lines:
    ...
    # If we are inside the dtrack-publisher-chart block, replace values
    if in_block:
...
kubectl rollout restart deployment -n $NAMESPACE -l app.kubernetes.io/name=dtrack-publisher-chart || true
