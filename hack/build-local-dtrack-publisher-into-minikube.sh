...
# Variables for minikube profile and dtrack-publisher
SBOM_SERVICE_IMAGE="dtrack-publisher:latest"
PROFILE="sbomer"
TAR_FILE="dtrack-publisher.tar"

echo "--- Building and inserting dtrack-publisher image into Minikube registry ---"

bash ./hack/build-with-schemas.sh prod

podman build --format docker -t "$SBOM_SERVICE_IMAGE" -f src/main/docker/Dockerfile.jvm .

echo "--- Exporting dtrack-publisher image to archive ---"
if [ -f "$TAR_FILE" ]; then
    rm "$TAR_FILE"
fi
podman save -o "$TAR_FILE" "$SBOM_SERVICE_IMAGE"

echo "--- Loading dtrack-publisher into Minikube ---"
